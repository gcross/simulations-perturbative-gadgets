-- @+leo-ver=4-thin
-- @+node:gcross.20091211100630.1316:@thin VMPSDatabase.hs
-- @@language Haskell

-- @<< Language extensions >>
-- @+node:gcross.20091211100630.1317:<< Language extensions >>
{-# LANGUAGE FlexibleContexts #-}
-- @-node:gcross.20091211100630.1317:<< Language extensions >>
-- @nl

module VMPSDatabase where

-- @<< Import needed modules >>
-- @+node:gcross.20091211100630.1318:<< Import needed modules >>
...
-- @+others
-- @+node:gcross.20091211100630.1319:Values
-- @+node:gcross.20091211100630.1320:digits
zero :: Word8
zero = 48
one = zero+1
two = one+1
...
-- @-node:gcross.20091211100630.1320:digits
-- @-node:gcross.20091211100630.1319:Values
-- @+node:gcross.20091211100630.1321:Enumerators
-- @+node:gcross.20091211100630.1322:getX
...
-- @-node:gcross.20091211100630.1322:getX
-- @+node:gcross.20091211100630.1323:fetchX
...
-- @-node:gcross.20091211100630.1323:fetchX
-- @-node:gcross.20091211100630.1321:Enumerators
-- @+node:gcross.20091211100630.1324:Functions
-- @+node:gcross.20091211100630.1325:makeConnection
...
-- @-node:gcross.20091211100630.1325:makeConnection
-- @+node:gcross.20091211100630.1326:toEncodedString
...
-- @-node:gcross.20091211100630.1326:toEncodedString
-- @+node:gcross.20091211100630.1327:fromEncodedString
...
-- @-node:gcross.20091211100630.1327:fromEncodedString
-- @+node:gcross.20091211100630.1328:withStateSiteTensorAsByteString
...
withStateSiteTensorAsByteString tensor thunk =
    ...
    in withPinnedTensor tensor $
        ...
        thunk
-- @-node:gcross.20091211100630.1328:withStateSiteTensorAsByteString
-- @+node:gcross.20091211100630.1329:decode
decode :: [Word8] -> [Word8]
decode [] = []
...
-- @-node:gcross.20091211100630.1329:decode
-- @+node:gcross.20091211100630.1330:encode
encode :: [Word8] -> [Word8]
encode [] = []
...
encode (other:rest)
    ...
    | otherwise
      = other:encode rest
-- @-node:gcross.20091211100630.1330:encode
-- @+node:gcross.20091211100630.1331:deslash
deslash :: [Word8] -> [Word8]
deslash [] = []
deslash (92:92:rest) = 92:deslash rest
deslash (other:rest) = other:deslash rest
-- @-node:gcross.20091211100630.1331:deslash
-- @+node:gcross.20091211100630.1332:stateIteratee
...
-- @-node:gcross.20091211100630.1334:fetchState
-- @+node:gcross.20091211100630.1335:insertRows
insertRows name statement type_ids rows =
    ...
    \number_of_rows_inserted ->
        unless (number_of_rows_inserted == length rows) . error $
            ...
            " rows."
-- @-node:gcross.20091211100630.1335:insertRows
-- @+node:gcross.20091211100630.1336:storeState
...
-- @-node:gcross.20091211100630.1336:storeState
-- @+node:gcross.20091211100630.1337:storeSolution
